...
{	
	...
	switch (m_scale) //set rect. pos. depending to scale
	{
		default: //TODO: set pos for each scale
			...
			for (int i=0; i < stats.players.size(); i++) //add as many lifebars outlines & backgrounds as needed
			{
			...
			for (int i=1; i <= stats.players.size(); i++) //add as many lifebars outlines as needed
...
void D3DHook::refreshLife()
{
	...
	for (int i=0; i < stats.players.size(); i++) //calc all lenghts of player's lifebars
		m_llives[i] = stats.players[i].health * m_lmlife / stats.players[i].maxHealth;

	for (int i = 0; i < 6*stats.players.size(); i++) //remove 6 vertices for each players (=> remove all lifebars)
		m_vertices.pop_back();

	//TODO: dependence on m_scale
	for (int i=0; i < stats.players.size(); i++) //add as many lifebars outlines as needed
		addLifeRect(7.0f + WIDTH / 14.0f, i * yoff + 100.0f + WIDTH / 14.0f, WIDTH - 2.0f * WIDTH / 14.0f, D3DCOLOR_ARGB(255, 136, 211, 103), i - 1); //HEALTH
...
void D3DHook::textHud()
{
	...
	for (int i = 0; i < stats.players.size(); i++) //create array containing life strings to draw
	{
		life[i] << std::fixed << std::setprecision(0) << stats.players[i].health << "/" << stats.players[i].maxHealth;
		lvl[i] << "LV. " << std::fixed << std::setprecision(0) << stats.players[i].level;
	}
	item << std::fixed << m_item << " ITEMS";

	//LIFE & OUTLINE
	for (int i = 0; i < stats.players.size(); i++) //draw life and level of each existing player
	{
	...
	dmg << "DMG:  " << std::setprecision(2) << stats.players[m_pSel].dmg;
	rate << "FIRERATE:  " << std::setprecision(2) << stats.players[m_pSel].rate;
	crit << "CRIT:  " << std::setprecision(2) << stats.players[m_pSel].crit;
	regen << "REGEN:  " << std::setprecision(2) << stats.players[m_pSel].regen;
	strength << "STRENGTH:  " << std::setprecision(2) << stats.players[m_pSel].strength;
